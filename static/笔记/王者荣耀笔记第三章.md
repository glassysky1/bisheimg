...
<template>
  ...
  <m-card :icon="icon" :title="title">
    ...
    <div class="pt-3">
      <!-- 把组件的index赋值给active -->
      <swiper ref="list" :options="{autoHeigth:true}" @slide-change="() => active = $refs.list.swiper.realIndex">
        <swiper-slide v-for="(category,index) in categories" :key="index">
          <slot name="items" :category="category"></slot>
        </swiper-slide>
      </swiper>
...
```javascript
  ...
  router.get('/heroes/init', async (req, res) => {
    ...
    for(let cat of rawData){
      ...
      //找到当期分类在数据库的对应的数据对应的数据
      const category = await Category.findOne({
        name:cat.name
      })
      cat.heroes = cat.heroes.map(hero =>{
      //  mongoose很智能，category传入时，会自动把id传进去
...
```javascript
  ...
  router.get('/heroes/list', async (req, res) => {
    ...
    //     "heroList": [
    //       {
    //         "_id": "5dcd48f3bf4372a1b8c2294b",
    //         "categories": [
    //           "5dc535a3c098473120686132"
    //         ],
    //         "items1": [],
    //         "items2": [],
    //         "name": "赵云",
...
```vue
   <m-list-card icon="menu-1" title="英雄列表" :categories="heroCats">
      <template #items="{category}">
